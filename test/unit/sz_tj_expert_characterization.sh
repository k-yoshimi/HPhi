...
WORKDIR="unit_sz_tj_expert_characterization_work"
HPHI_BIN="../../../src/HPhi"
...
cp ../trans.def .
cp ../namelist.def .
"${HPHI_BIN}" -e namelist.def > tj_sz.log 2>&1
grep -q '^sdim=16 =2\^4$' output/CHECK_Sdim.dat
grep -Eq 'idim_max=12([[:space:]]|$)' output/CHECK_Memory.dat
...
cp ../trans.def .
cp ../namelist.def .
"${HPHI_BIN}" -e namelist.def > tj_ncons.log 2>&1
grep -q '^sdim=16 =2\^4$' output/CHECK_Sdim.dat
grep -Eq 'idim_max=32([[:space:]]|$)' output/CHECK_Memory.dat
test ! -f output/Err_sz.dat

echo "PASS: unit_sz_tj_expert_characterization"
